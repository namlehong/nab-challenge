...
describe("Test 'products' service", () => {
	...
	describe("Test actions", () => {
		...
		const analyticService = broker.createService(AnalyticService);
		const service = broker.createService(TestService);
		service.seedDB = null; // Disable seeding
		analyticService.seedDB = null; // Disable seeding
		...
		it("should get the saved item", async () => {
			...
			expect(res2).toEqual({
				page: 1,
				pageSize: 10,
				rows: [{ _id: newID, isSale: false, name: "Awesome item", price: 999, quantity: 0 }],
				total: 1,
				totalPages: 1
			});
		});
